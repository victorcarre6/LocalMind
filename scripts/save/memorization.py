...
def get_relevant_context(user_question, limit=3, recall=True):
    ...
    filtered_context = []
    for (user_input, llm_output, llm_model, timestamp, convo_id) in context_rows:
...
def generate_prompt_paragraph(context, question, keywords=None, lang=None, history_limit=0):
    ...
    for idx, item in enumerate(context[:limit], start=1):
        try:
            ...
            items_to_summarize.append(compressed_output)
            processed_items_meta.append((idx, user_input))
        except Exception as e:
    ...
    if items_to_summarize:
        ...
        for meta, summary in zip(processed_items_meta, summaries):
            idx, user_input = meta
    ...
    last_convos = get_last_conversations(limit=history_limit)
    last_items_to_summarize = []
    processed_last_convos_meta = []
    for idx, (user_input, llm_output) in enumerate(last_convos, start=1):
        try:
            shortened_user_input = str(user_input)[:300]
            cleaned_output = format_cleaner(str(llm_output))
            compressed_output = compress_text(cleaned_output)
            last_items_to_summarize.append(compressed_output)
            processed_last_convos_meta.append((idx, shortened_user_input))
        except Exception as e:
            print(f"Erreur traitement item : {e}")
            continue
    processed_last_convos = []
    if last_items_to_summarize:
        last_summaries = summarize_batch(last_items_to_summarize)
        for meta, summary in zip(processed_last_convos_meta, last_summaries):
            idx, shortened_user_input = meta
